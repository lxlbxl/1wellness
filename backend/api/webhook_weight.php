...
if (!$hashOk) {
    $integrity->log('webhook', 'rejected', ['detail' => ['source' => 'webhook_weight', 'reason' => $hashReason]]);
...
if ($txId) {
    ...
    if (abs((float)($verified['amount'] ?? 0) - (float)($input['amount'] ?? 0)) > 0.01) {
        $integrity->log('webhook', 'rejected', ['tx_ref' => $txRef, 'transaction_id' => $txId,
            'detail' => ['source' => 'webhook_weight', 'reason' => 'amount_mismatch',
...
try {
    ...
    $product = $input['product'] ?? 'Weight Management Bundle';
    ...
    $assessmentData = [
        'weight_type' => $input['weight_type'] ?? 'General',
        'current_weight' => $input['current_weight'] ?? 0,
        'target_weight' => $input['target_weight'] ?? 0,
        'allergies' => $input['allergies'] ?? 'None'
    ];

    $result = $orchestrator->handleWeightPurchase($orderData, $assessmentData);
